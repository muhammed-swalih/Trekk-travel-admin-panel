...
function DltHoneymoon() {
    ...
    const fetchHoneymoon = async () => {
        const response = await axios.get('https://trekkbackend.fun/packages/getAd')
        setHoneymoon(response.data)

    }

    const handleDelete = (id) => {
        fetch(`https://trekkbackend.fun/packages/deleteAd/${id}`, {
            method: "DELETE"
        })
    ...
    return (
        <div className=' w-full min-h-screen h-auto bg-cover bg-gray-900 pt-10' style={{ backgroundImage: `url(${bg})` }}>
            ...
            <div className=' w-w-[1150px] h-auto  px-10'>
