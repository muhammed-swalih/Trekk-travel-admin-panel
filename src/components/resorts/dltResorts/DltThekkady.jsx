...
function DltThekkady() {
  const [honeymoon, setHoneymoon] = useState([]);

  const fetchHoneymoon = async () => {
    const response = await axios.get('http://localhost:3001/packages/thekkady')
    setHoneymoon(response.data)

  }

  const handleDelete = (id) => {
    fetch(`http://localhost:3001/packages/deletethekkadyresort/${id}`, {
      method: "DELETE"
    })
  ...
  return (
    <div className=' w-full min-h-screen h-auto bg-cover bg-gray-900 pt-10' style={{ backgroundImage: `url(${bg})` }}>
      <div className='  py-[100px]'>
        <h1 className=' text-[100px] uppercase text-white text-center'>delete thekkady resorts</h1>
