...
function DltVagamon() {
  ...
  const fetchHoneymoon = async () => {
    const response = await axios.get('http://trekkbackend.fun/packages/vagamon')
    setHoneymoon(response.data)

  }

  const handleDelete = (id) => {
    fetch(`http://trekkbackend.fun/packages/deletevagamonresort/${id}`, {
      method: "DELETE"
    })
  ...
  return (
    <div className=' w-full min-h-screen h-auto bg-cover bg-gray-900 pt-10' style={{ backgroundImage: `url(${bg})` }}>
            <div className='  py-1'>
                <h1 className=' text-3xl uppercase text-white text-center'>delete vagamon packages</h1>
