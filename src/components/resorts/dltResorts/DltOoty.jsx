...
function DltOoty() {
  ...
  const fetchHoneymoon = async () => {
    const response = await axios.get('https://trekkbackend.fun/packages/ooty')
    setHoneymoon(response.data)

  }

  const handleDelete = (id) => {
    fetch(`https://trekkbackend.fun/packages/deleteootyresort/${id}`, {
      method: "DELETE"
    })
  ...
  return (
    <div className=' w-full min-h-screen h-auto bg-cover bg-gray-900 pt-10' style={{ backgroundImage: `url(${bg})` }}>
            <div className='  py-1'>
                <h1 className=' text-3xl uppercase text-white text-center'>delete ooty packages</h1>
