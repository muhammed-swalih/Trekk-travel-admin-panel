...
function TrivandrumForm() {
    ...
    const handleSubmit = async (event) => {
        ...
        if(place.length ==0 ||  description.length==0 || testImage.length ==0){
            ...
            try {
                const response = await fetch("https://trekkbackend.fun/packages/trivandrumresort", {
                    method: "POST",
                    body: formData,
                });
    ...
    return (
        ...
        <h1 className='uppercase text-2xl font-medium text-white text-center '>add trivandrum package</h1>
