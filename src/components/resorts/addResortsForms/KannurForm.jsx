...
function KannurForm() {
    ...
    const handleSubmit = async (event) => {
        if(place.length ==0 || description.length==0 || testImage.length ==0){
            ...
            try {
                const response = await fetch("http://localhost:3001/packages/kannurresort", {
                    method: "POST",
                    body: formData,
                });
    ...
    return (
        ...
        <div className='  py-[100px]'>
                <h1 className=' text-[100px] uppercase text-white text-center'>add kannur resort</h1>
