...
function ThekkadyForm() {
    ...
    const handleSubmit = async (event) => {
        ...
        if(place.length ==0 ||  description.length==0 || testImage.length ==0){
            ...
            try {
                const response = await fetch("https://trekkbackend.fun/packages/thekkadyresort", {
                    method: "POST",
                    body: formData,
                });
    ...
    return (
        <div className=' w-full min-h-screen h-auto bg-gray-900 bg-cover pt-16' style={{ backgroundImage: `url(${bg})` }}>
            <h1 className='uppercase text-2xl font-medium text-white text-center '>add thekkady package</h1>
