...
function HoneyMoon() {
    ...
    const handleSubmit = async (event) => {
        if (place.length == 0 || price.length == 0 || days.length == 0 || highlights1.length == 0 || description.length == 0 || testImage.length == 0) {
            ...
            try {
                const response = await fetch("https://trekkandtravel.onrender.com/packages/honeymoonPackage", {
                    method: "POST",
                    body: formData,
                });
    ...
    return (
        <div className=' w-full min-h-screen h-auto bg-gray-900 bg-cover pt-16' style={{ backgroundImage: `url(${bg})` }}>
            <h1 className='uppercase text-2xl font-medium text-white text-center '>add honeymoon package</h1>
